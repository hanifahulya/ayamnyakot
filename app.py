import streamlit as st
import pandas as pd
from pathlib import Path
from datetime import datetime, date
import random
import io

# =========================================================
# AYAM NYAKOT - CUSTOMER ORDER & ADMIN TRACKING
# Web pelanggan untuk order online + web admin untuk kelola pesanan
# =========================================================

st.set_page_config(
    page_title="Ayam Nyakot",
    page_icon="🍗",
    layout="wide",
    initial_sidebar_state="expanded"
)

DATA_DIR = Path("data")
DATA_FILE = DATA_DIR / "ayam_nyakot_delivery.csv"

STATUS_OPTIONS = [
    "Pesanan Dibuat",
    "Disiapkan",
    "Dalam Pengantaran",
    "Terkirim",
    "Selesai",
    "Dibatalkan"
]

PAYMENT_OPTIONS = [
    "Belum Dibayar",
    "Sudah Dibayar"
]

ORDER_METHOD_OPTIONS = [
    "Diantar Kurir Internal",
    "Diantar Ojek Online",
    "Ambil Sendiri",
    "Beli Langsung di Tempat",
    "Lainnya"
]

PAKET_MENU = {
    "Ayam Geprek Tanpa Nasi": 10000,
    "Ayam Geprek Pakai Nasi": 15000
}

TINGKAT_PEDAS = [
    "Tidak Pedas",
    "Pedas"
]

COLUMNS = [
    "order_id",
    "tanggal",
    "sumber_pesanan",
    "nama_pelanggan",
    "no_hp",
    "alamat_pengantaran",
    "menu_pesanan",
    "jumlah",
    "total_pesanan",
    "ongkir",
    "metode_pesanan",
    "nama_petugas",
    "status_pengantaran",
    "status_pembayaran_ongkir",
    "estimasi_sampai",
    "catatan",
    "updated_at"
]


# =========================
# STYLE / CSS
# =========================
st.markdown(
    """
    <style>
    .main {
        background: linear-gradient(180deg, #fffaf3 0%, #ffffff 35%);
    }

    .block-container {
        padding-top: 1.4rem;
        padding-bottom: 2rem;
    }

    .hero {
        padding: 28px 30px;
        border-radius: 30px;
        background: linear-gradient(135deg, #ff8c00 0%, #ff4d00 55%, #7a1d00 100%);
        color: white;
        box-shadow: 0 18px 42px rgba(255, 91, 0, .24);
        margin-bottom: 22px;
    }

    .hero h1 {
        font-size: 2.5rem;
        margin-bottom: 6px;
    }

    .hero p {
        font-size: 1.05rem;
        opacity: .96;
        margin: 0;
    }

    .customer-hero {
        padding: 34px 32px;
        border-radius: 32px;
        background: linear-gradient(135deg, #fb923c 0%, #ef4444 55%, #7f1d1d 100%);
        color: white;
        box-shadow: 0 18px 42px rgba(239, 68, 68, .23);
        margin-bottom: 22px;
    }

    .customer-hero h1 {
        font-size: 2.7rem;
        margin-bottom: 8px;
        font-weight: 950;
    }

    .customer-hero p {
        font-size: 1.08rem;
        margin: 0;
        opacity: .96;
    }

    .menu-price-card {
        padding: 18px 20px;
        border-radius: 22px;
        background: #fff7ed;
        border: 1px solid #fed7aa;
        margin-bottom: 12px;
    }

    .menu-title {
        font-weight: 900;
        color: #431407;
        font-size: 1.1rem;
    }

    .menu-price {
        color: #ea580c;
        font-weight: 900;
        font-size: 1.4rem;
    }

    .product-card {
        padding: 24px;
        border-radius: 28px;
        background: #ffffff;
        border: 1px solid rgba(251, 146, 60, .28);
        box-shadow: 0 12px 34px rgba(35, 35, 35, .07);
        height: 100%;
    }

    .product-title {
        font-size: 1.35rem;
        font-weight: 950;
        color: #431407;
        margin-bottom: 4px;
    }

    .product-desc {
        color: #57534e;
        font-size: .95rem;
        margin-bottom: 10px;
    }

    .product-price {
        color: #ea580c;
        font-size: 1.65rem;
        font-weight: 950;
        margin-top: 8px;
    }

    .metric-card {
        padding: 18px;
        border-radius: 22px;
        background: #fff7ed;
        border: 1px solid #fed7aa;
        height: 100%;
    }

    .metric-label {
        color: #7c2d12;
        font-size: .9rem;
        font-weight: 700;
    }

    .metric-value {
        color: #431407;
        font-size: 1.7rem;
        font-weight: 900;
        margin-top: 2px;
    }

    .status-pill {
        display: inline-block;
        padding: 6px 12px;
        border-radius: 999px;
        font-size: .82rem;
        font-weight: 800;
    }

    .pill-progress { background: #dbeafe; color: #1e40af; }
    .pill-done { background: #dcfce7; color: #166534; }
    .pill-unpaid { background: #fee2e2; color: #991b1b; }
    .pill-paid { background: #dcfce7; color: #166534; }
    .pill-cancel { background: #f3f4f6; color: #374151; }

    div[data-testid="stMetricValue"] {
        font-weight: 900;
    }

    .footer-box {
        padding: 18px;
        border-radius: 18px;
        background: #292524;
        color: white;
        margin-top: 24px;
    }

    .order-success {
        padding: 22px;
        border-radius: 22px;
        background: #dcfce7;
        border: 1px solid #86efac;
        color: #14532d;
    }
    </style>
    """,
    unsafe_allow_html=True
)


# =========================
# HELPER FUNCTIONS
# =========================
def rupiah(value):
    try:
        value = int(float(value))
        return f"Rp{value:,.0f}".replace(",", ".")
    except Exception:
        return "Rp0"


def now_string():
    return datetime.now().strftime("%Y-%m-%d %H:%M:%S")


def generate_order_id():
    today = datetime.now().strftime("%Y%m%d")
    suffix = random.randint(1000, 9999)
    return f"AN-{today}-{suffix}"


def ensure_data_file():
    DATA_DIR.mkdir(exist_ok=True)
    if not DATA_FILE.exists():
        sample = pd.DataFrame(
            [
                {
                    "order_id": "AN-20260508-1001",
                    "tanggal": "2026-05-08",
                    "sumber_pesanan": "Admin",
                    "nama_pelanggan": "Rina",
                    "no_hp": "081234567890",
                    "alamat_pengantaran": "Jl. Merdeka No. 10",
                    "menu_pesanan": "Ayam Geprek Pakai Nasi - Pedas",
                    "jumlah": 2,
                    "total_pesanan": 30000,
                    "ongkir": 8000,
                    "metode_pesanan": "Diantar Kurir Internal",
                    "nama_petugas": "Bang Andi",
                    "status_pengantaran": "Dalam Pengantaran",
                    "status_pembayaran_ongkir": "Belum Dibayar",
                    "estimasi_sampai": "18:30",
                    "catatan": "Tolong antar ke pos satpam.",
                    "updated_at": now_string(),
                },
                {
                    "order_id": "AN-20260508-1002",
                    "tanggal": "2026-05-08",
                    "sumber_pesanan": "Pelanggan Online",
                    "nama_pelanggan": "Dimas",
                    "no_hp": "082111223344",
                    "alamat_pengantaran": "Kampus / Gedung B",
                    "menu_pesanan": "Ayam Geprek Tanpa Nasi - Tidak Pedas",
                    "jumlah": 1,
                    "total_pesanan": 10000,
                    "ongkir": 0,
                    "metode_pesanan": "Diantar Kurir Internal",
                    "nama_petugas": "-",
                    "status_pengantaran": "Pesanan Dibuat",
                    "status_pembayaran_ongkir": "Belum Dibayar",
                    "estimasi_sampai": "-",
                    "catatan": "Ongkir akan dikonfirmasi admin.",
                    "updated_at": now_string(),
                },
                {
                    "order_id": "AN-20260508-1003",
                    "tanggal": "2026-05-08",
                    "sumber_pesanan": "Admin",
                    "nama_pelanggan": "Salsa",
                    "no_hp": "085266778899",
                    "alamat_pengantaran": "-",
                    "menu_pesanan": "Ayam Geprek Pakai Nasi - Pedas",
                    "jumlah": 3,
                    "total_pesanan": 45000,
                    "ongkir": 0,
                    "metode_pesanan": "Beli Langsung di Tempat",
                    "nama_petugas": "Kasir",
                    "status_pengantaran": "Selesai",
                    "status_pembayaran_ongkir": "Sudah Dibayar",
                    "estimasi_sampai": "-",
                    "catatan": "Pembelian offline.",
                    "updated_at": now_string(),
                },
            ],
            columns=COLUMNS
        )
        sample.to_csv(DATA_FILE, index=False)


@st.cache_data(show_spinner=False)
def load_data():
    ensure_data_file()
    df = pd.read_csv(DATA_FILE)

    # Kompatibilitas jika file CSV lama belum punya kolom baru
    if "sumber_pesanan" not in df.columns:
        df["sumber_pesanan"] = "Admin"
    if "metode_kurir" in df.columns and "metode_pesanan" not in df.columns:
        df["metode_pesanan"] = df["metode_kurir"]
    if "nama_kurir" in df.columns and "nama_petugas" not in df.columns:
        df["nama_petugas"] = df["nama_kurir"]

    for col in COLUMNS:
        if col not in df.columns:
            df[col] = ""

    df = df[COLUMNS]
    df["jumlah"] = pd.to_numeric(df["jumlah"], errors="coerce").fillna(0).astype(int)
    df["total_pesanan"] = pd.to_numeric(df["total_pesanan"], errors="coerce").fillna(0).astype(int)
    df["ongkir"] = pd.to_numeric(df["ongkir"], errors="coerce").fillna(0).astype(int)
    return df


def save_data(df):
    DATA_DIR.mkdir(exist_ok=True)
    df = df[COLUMNS]
    df.to_csv(DATA_FILE, index=False)
    st.cache_data.clear()


def add_order(new_row):
    df = load_data()
    df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
    save_data(df)


def update_order(order_id, fields):
    df = load_data()
    idx = df.index[df["order_id"] == order_id]
    if len(idx) == 0:
        return False
    for key, value in fields.items():
        df.loc[idx, key] = value
    df.loc[idx, "updated_at"] = now_string()
    save_data(df)
    return True


def delete_order(order_id):
    df = load_data()
    df = df[df["order_id"] != order_id]
    save_data(df)


def get_status_badge(status):
    if status in ["Selesai", "Terkirim"]:
        return f"<span class='status-pill pill-done'>{status}</span>"
    if status == "Dibatalkan":
        return f"<span class='status-pill pill-cancel'>{status}</span>"
    return f"<span class='status-pill pill-progress'>{status}</span>"


def get_payment_badge(status):
    if status == "Sudah Dibayar":
        return f"<span class='status-pill pill-paid'>{status}</span>"
    return f"<span class='status-pill pill-unpaid'>{status}</span>"


def make_csv_download(df):
    return df.to_csv(index=False).encode("utf-8")


def make_excel_download(df):
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        df.to_excel(writer, index=False, sheet_name="Tracking Ayam Nyakot")
    return output.getvalue()


def render_product_cards():
    c1, c2 = st.columns(2)
    with c1:
        st.markdown(
            """
            <div class="product-card">
                <div style="font-size: 2.4rem;">🍗</div>
                <div class="product-title">Ayam Geprek Tanpa Nasi</div>
                <div class="product-desc">Ayam geprek pedas gurih tanpa nasi. Cocok untuk lauk atau camilan berat.</div>
                <div class="product-price">Rp10.000</div>
            </div>
            """,
            unsafe_allow_html=True
        )
    with c2:
        st.markdown(
            """
            <div class="product-card">
                <div style="font-size: 2.4rem;">🍚</div>
                <div class="product-title">Ayam Geprek Pakai Nasi</div>
                <div class="product-desc">Paket ayam geprek lengkap dengan nasi. Cocok untuk makan siang atau malam.</div>
                <div class="product-price">Rp15.000</div>
            </div>
            """,
            unsafe_allow_html=True
        )


# =========================
# APP MODE
# =========================
with st.sidebar:
    st.markdown("## 🍗 Ayam Nyakot")
    st.caption("Order Online & Admin Tracking")
    st.divider()

    app_mode = st.radio(
        "Masuk sebagai",
        [
            "Pelanggan",
            "Admin / Owner"
        ]
    )

    st.divider()
    st.markdown("### Menu Ayam Nyakot")
    st.markdown(
        """
        <div class="menu-price-card">
            <div class="menu-title">Ayam Geprek Tanpa Nasi</div>
            <div class="menu-price">Rp10.000</div>
        </div>
        <div class="menu-price-card">
            <div class="menu-title">Ayam Geprek Pakai Nasi</div>
            <div class="menu-price">Rp15.000</div>
        </div>
        """,
        unsafe_allow_html=True
    )

df = load_data()


# =========================================================
# CUSTOMER MODE
# =========================================================
if app_mode == "Pelanggan":
    st.markdown(
        """
        <div class="customer-hero">
            <h1>Pesan Ayam Geprek Nyakot 🍗</h1>
            <p>Pilih menu, isi data pesanan, lalu simpan order. Admin akan memproses pesananmu.</p>
        </div>
        """,
        unsafe_allow_html=True
    )

    tab_order, tab_tracking = st.tabs(["🛒 Pesan Sekarang", "🔎 Cek Pesanan"])

    with tab_order:
        st.markdown("## Menu")
        render_product_cards()

        st.divider()
        st.markdown("## Form Order Online")

        with st.form("form_order_pelanggan", clear_on_submit=True):
            c1, c2 = st.columns(2)

            with c1:
                nama_pelanggan = st.text_input("Nama Lengkap")
                no_hp = st.text_input("Nomor WhatsApp")
                paket = st.selectbox("Pilih Menu", list(PAKET_MENU.keys()))
                tingkat_pedas = st.selectbox("Tingkat Rasa", TINGKAT_PEDAS)
                jumlah = st.number_input("Jumlah Porsi", min_value=1, value=1, step=1)

                harga_satuan = PAKET_MENU[paket]
                total_pesanan = harga_satuan * int(jumlah)

                st.info(f"Harga satuan: {rupiah(harga_satuan)}")
                st.success(f"Total pesanan: {rupiah(total_pesanan)}")

            with c2:
                metode_pesanan_customer = st.selectbox(
                    "Metode Pesanan",
                    [
                        "Diantar ke Alamat",
                        "Ambil Sendiri ke Tempat"
                    ]
                )

                if metode_pesanan_customer == "Diantar ke Alamat":
                    alamat_pengantaran = st.text_area("Alamat Lengkap Pengantaran")
                    metode_pesanan = "Diantar Kurir Internal"
                    ongkir = 0
                    st.warning("Ongkir belum termasuk total pesanan dan akan dikonfirmasi oleh admin.")
                    catatan_placeholder = "Contoh: rumah pagar hitam / antar sore / chat dulu sebelum antar"
                else:
                    alamat_pengantaran = "-"
                    metode_pesanan = "Ambil Sendiri"
                    ongkir = 0
                    st.info("Pesanan diambil sendiri, sehingga tidak ada ongkir.")
                    catatan_placeholder = "Contoh: diambil jam 17.00"

                catatan = st.text_area("Catatan Tambahan", placeholder=catatan_placeholder)

            submitted = st.form_submit_button("Kirim Pesanan", type="primary", use_container_width=True)

            if submitted:
                if not nama_pelanggan or not no_hp:
                    st.error("Nama dan nomor WhatsApp wajib diisi.")
                elif metode_pesanan_customer == "Diantar ke Alamat" and not alamat_pengantaran:
                    st.error("Alamat pengantaran wajib diisi untuk pesanan antar.")
                else:
                    order_id = generate_order_id()
                    menu_pesanan = f"{paket} - {tingkat_pedas}"

                    if metode_pesanan_customer == "Diantar ke Alamat":
                        catatan_final = catatan if catatan else "Ongkir akan dikonfirmasi admin."
                    else:
                        catatan_final = catatan if catatan else "Pesanan diambil sendiri oleh pelanggan."

                    new_order = {
                        "order_id": order_id,
                        "tanggal": str(date.today()),
                        "sumber_pesanan": "Pelanggan Online",
                        "nama_pelanggan": nama_pelanggan,
                        "no_hp": no_hp,
                        "alamat_pengantaran": alamat_pengantaran,
                        "menu_pesanan": menu_pesanan,
                        "jumlah": int(jumlah),
                        "total_pesanan": int(total_pesanan),
                        "ongkir": int(ongkir),
                        "metode_pesanan": metode_pesanan,
                        "nama_petugas": "-",
                        "status_pengantaran": "Pesanan Dibuat",
                        "status_pembayaran_ongkir": "Belum Dibayar",
                        "estimasi_sampai": "-",
                        "catatan": catatan_final,
                        "updated_at": now_string()
                    }
                    add_order(new_order)

                    st.markdown(
                        f"""
                        <div class="order-success">
                            <h3>Pesanan berhasil dikirim ✅</h3>
                            <p><b>Order ID:</b> {order_id}</p>
                            <p><b>Nama:</b> {nama_pelanggan}</p>
                            <p><b>Menu:</b> {menu_pesanan}</p>
                            <p><b>Jumlah:</b> {jumlah} porsi</p>
                            <p><b>Total Pesanan:</b> {rupiah(total_pesanan)}</p>
                            <p>Simpan Order ID ini untuk mengecek status pesanan.</p>
                        </div>
                        """,
                        unsafe_allow_html=True
                    )

    with tab_tracking:
        st.markdown("## Cek Status Pesanan")
        st.caption("Masukkan Order ID dan nomor WhatsApp yang dipakai saat memesan.")

        cek_order_id = st.text_input("Order ID", placeholder="Contoh: AN-20260508-1234")
        cek_no_hp = st.text_input("Nomor WhatsApp")

        if st.button("Cek Pesanan", type="primary"):
            if not cek_order_id or not cek_no_hp:
                st.error("Order ID dan nomor WhatsApp wajib diisi.")
            else:
                result = df[
                    (df["order_id"].astype(str).str.lower() == cek_order_id.lower()) &
                    (df["no_hp"].astype(str) == cek_no_hp)
                ]

                if result.empty:
                    st.warning("Pesanan tidak ditemukan. Pastikan Order ID dan nomor WhatsApp sudah benar.")
                else:
                    row = result.iloc[0]

                    with st.container(border=True):
                        top1, top2 = st.columns([2, 1])
                        with top1:
                            st.markdown(f"### {row['order_id']}")
                            st.write(f"**Nama:** {row['nama_pelanggan']}")
                            st.write(f"**Menu:** {row['menu_pesanan']}")
                            st.write(f"**Jumlah:** {row['jumlah']} porsi")
                        with top2:
                            st.markdown(get_status_badge(row["status_pengantaran"]), unsafe_allow_html=True)
                            st.markdown(get_payment_badge(row["status_pembayaran_ongkir"]), unsafe_allow_html=True)

                        st.divider()
                        c1, c2, c3 = st.columns(3)
                        with c1:
                            st.write("**Total Pesanan:**", rupiah(row["total_pesanan"]))
                            st.write("**Ongkir:**", rupiah(row["ongkir"]))
                            st.write("**Total Bayar:**", rupiah(row["total_pesanan"] + row["ongkir"]))
                        with c2:
                            st.write("**Metode Pesanan:**", row["metode_pesanan"])
                            st.write("**Estimasi:**", row["estimasi_sampai"])
                            st.write("**Alamat:**", row["alamat_pengantaran"])
                        with c3:
                            st.write("**Catatan Admin/Pesanan:**", row["catatan"])
                            st.write("**Terakhir Update:**", row["updated_at"])

                    st.info("Jika ada perubahan alamat atau ingin membatalkan pesanan, silakan hubungi admin Ayam Nyakot.")


# =========================================================
# ADMIN MODE
# =========================================================
else:
    st.markdown(
        """
        <div class="hero">
            <h1>🍗 Ayam Nyakot Admin</h1>
            <p>Dashboard admin untuk mengelola pesanan online, pembelian langsung, tracking pesanan, dan pembayaran ongkir.</p>
        </div>
        """,
        unsafe_allow_html=True
    )

    # PIN sederhana untuk demo, supaya halaman admin tidak langsung terbuka untuk pelanggan.
    if "admin_login" not in st.session_state:
        st.session_state.admin_login = False

    if not st.session_state.admin_login:
        st.markdown("## Login Admin")
        st.caption("Masukkan PIN untuk mengakses halaman.")
        pin = st.text_input("Masukkan PIN Admin", type="password")
        if st.button("Masuk Admin", type="primary"):
            if pin == "1234":
                st.session_state.admin_login = True
                st.success("Berhasil masuk sebagai admin.")
                st.rerun()
            else:
                st.error("PIN salah.")
        st.stop()

    with st.sidebar:
        st.divider()
        admin_page = st.radio(
            "Menu Admin",
            [
                "Dashboard",
                "Tambah Pesanan",
                "Tracking Pesanan",
                "Update Status",
                "Data & Export"
            ]
        )

        if st.button("Keluar Admin"):
            st.session_state.admin_login = False
            st.rerun()

    df = load_data()

    # =========================
    # ADMIN PAGE: DASHBOARD
    # =========================
    if admin_page == "Dashboard":
        total_order = len(df)
        online_order = len(df[df["sumber_pesanan"] == "Pelanggan Online"])
        ongoing_order = len(df[df["status_pengantaran"].isin(["Pesanan Dibuat", "Disiapkan", "Dalam Pengantaran"])])
        done_order = len(df[df["status_pengantaran"].isin(["Terkirim", "Selesai"])])
        unpaid_ongkir = len(df[df["status_pembayaran_ongkir"] == "Belum Dibayar"])
        total_omzet = df.loc[df["status_pengantaran"] != "Dibatalkan", "total_pesanan"].sum()
        total_ongkir = df["ongkir"].sum()
        paid_ongkir_value = df.loc[df["status_pembayaran_ongkir"] == "Sudah Dibayar", "ongkir"].sum()
        unpaid_ongkir_value = df.loc[df["status_pembayaran_ongkir"] == "Belum Dibayar", "ongkir"].sum()

        col1, col2, col3, col4, col5 = st.columns(5)
        with col1:
            st.markdown(f"<div class='metric-card'><div class='metric-label'>Total Pesanan</div><div class='metric-value'>{total_order}</div></div>", unsafe_allow_html=True)
        with col2:
            st.markdown(f"<div class='metric-card'><div class='metric-label'>Pesanan Online</div><div class='metric-value'>{online_order}</div></div>", unsafe_allow_html=True)
        with col3:
            st.markdown(f"<div class='metric-card'><div class='metric-label'>Sedang Diproses</div><div class='metric-value'>{ongoing_order}</div></div>", unsafe_allow_html=True)
        with col4:
            st.markdown(f"<div class='metric-card'><div class='metric-label'>Selesai/Terkirim</div><div class='metric-value'>{done_order}</div></div>", unsafe_allow_html=True)
        with col5:
            st.markdown(f"<div class='metric-card'><div class='metric-label'>Ongkir Belum Dibayar</div><div class='metric-value'>{unpaid_ongkir}</div></div>", unsafe_allow_html=True)

        st.markdown("### Ringkasan Penjualan & Ongkir")
        col_a, col_b, col_c, col_d = st.columns(4)
        col_a.metric("Total Omzet Pesanan", rupiah(total_omzet))
        col_b.metric("Total Ongkir", rupiah(total_ongkir))
        col_c.metric("Ongkir Sudah Dibayar", rupiah(paid_ongkir_value))
        col_d.metric("Ongkir Belum Dibayar", rupiah(unpaid_ongkir_value))

        st.markdown("### Pesanan Terbaru")
        if df.empty:
            st.info("Belum ada data pesanan.")
        else:
            latest = df.sort_values("updated_at", ascending=False).head(10).copy()
            latest["total_pesanan"] = latest["total_pesanan"].apply(rupiah)
            latest["ongkir"] = latest["ongkir"].apply(rupiah)
            st.dataframe(
                latest[
                    [
                        "order_id",
                        "tanggal",
                        "sumber_pesanan",
                        "nama_pelanggan",
                        "menu_pesanan",
                        "jumlah",
                        "total_pesanan",
                        "status_pengantaran",
                        "status_pembayaran_ongkir",
                        "ongkir",
                        "updated_at"
                    ]
                ],
                use_container_width=True,
                hide_index=True
            )

        st.markdown("### Grafik Status Pesanan")
        status_count = df["status_pengantaran"].value_counts().reset_index()
        status_count.columns = ["Status", "Jumlah"]
        if not status_count.empty:
            st.bar_chart(status_count.set_index("Status"))

        st.markdown(
            """
            <div class="footer-box">
            <b>Catatan admin:</b> Pesanan online yang baru masuk biasanya memiliki ongkir Rp0 terlebih dahulu. Admin dapat memperbarui ongkir dan estimasi sampai melalui menu Update Status.
            </div>
            """,
            unsafe_allow_html=True
        )

    # =========================
    # ADMIN PAGE: TAMBAH PESANAN
    # =========================
    elif admin_page == "Tambah Pesanan":
        st.markdown("## ➕ Tambah Pesanan Manual")
        st.caption("Gunakan form ini untuk pesanan dari admin, pembelian langsung, atau pesanan yang masuk dari chat.")

        with st.form("form_tambah_pesanan_admin", clear_on_submit=True):
            c1, c2 = st.columns(2)

            with c1:
                tanggal = st.date_input("Tanggal Pesanan", value=date.today())
                nama_pelanggan = st.text_input("Nama Pelanggan")
                no_hp = st.text_input("Nomor HP / WhatsApp")
                alamat_pengantaran = st.text_area("Alamat Pengantaran")

                paket = st.selectbox("Pilih Menu", list(PAKET_MENU.keys()))
                tingkat_pedas = st.selectbox("Tingkat Rasa", TINGKAT_PEDAS)
                harga_satuan = PAKET_MENU[paket]
                jumlah = st.number_input("Jumlah Porsi", min_value=1, value=1, step=1)

                total_pesanan = harga_satuan * int(jumlah)

                st.info(f"Harga satuan: {rupiah(harga_satuan)}")
                st.success(f"Total harga pesanan otomatis: {rupiah(total_pesanan)}")

            with c2:
                metode_pesanan = st.selectbox("Metode Pesanan", ORDER_METHOD_OPTIONS)

                if metode_pesanan == "Beli Langsung di Tempat":
                    ongkir = 0
                    st.info("Pembelian langsung di tempat tidak menggunakan ongkir.")
                    nama_petugas = st.text_input("Nama Petugas/Kasir", placeholder="Contoh: Admin/Kasir")
                    estimasi_sampai = "-"
                    default_status_index = STATUS_OPTIONS.index("Selesai")
                    default_payment_index = PAYMENT_OPTIONS.index("Sudah Dibayar")
                elif metode_pesanan == "Ambil Sendiri":
                    ongkir = 0
                    st.info("Pesanan diambil sendiri, sehingga ongkir otomatis Rp0.")
                    nama_petugas = st.text_input("Nama Petugas", placeholder="Contoh: Admin")
                    estimasi_sampai = st.text_input("Estimasi Siap Diambil", placeholder="Contoh: 18:30")
                    default_status_index = STATUS_OPTIONS.index("Disiapkan")
                    default_payment_index = PAYMENT_OPTIONS.index("Sudah Dibayar")
                else:
                    ongkir = st.number_input("Ongkir / Bill Pengantaran (Rp)", min_value=0, value=0, step=1000)
                    nama_petugas = st.text_input("Nama Kurir / Driver")
                    estimasi_sampai = st.text_input("Estimasi Sampai", placeholder="Contoh: 18:30")
                    default_status_index = 0
                    default_payment_index = 0

                status_pengantaran = st.selectbox("Status Pesanan", STATUS_OPTIONS, index=default_status_index)
                status_pembayaran_ongkir = st.selectbox("Status Pembayaran Ongkir", PAYMENT_OPTIONS, index=default_payment_index)
                catatan = st.text_area("Catatan", placeholder="Contoh: ongkir dibayar saat pesanan sampai")

                total_bayar = total_pesanan + int(ongkir)
                st.warning(f"Total bayar termasuk ongkir: {rupiah(total_bayar)}")

            submitted = st.form_submit_button("Simpan Pesanan", type="primary", use_container_width=True)

            if submitted:
                if not nama_pelanggan or not no_hp:
                    st.error("Nama pelanggan dan nomor HP wajib diisi.")
                elif metode_pesanan not in ["Beli Langsung di Tempat", "Ambil Sendiri"] and not alamat_pengantaran:
                    st.error("Alamat wajib diisi untuk pesanan pengantaran.")
                else:
                    menu_pesanan = f"{paket} - {tingkat_pedas}"

                    new_order = {
                        "order_id": generate_order_id(),
                        "tanggal": str(tanggal),
                        "sumber_pesanan": "Admin",
                        "nama_pelanggan": nama_pelanggan,
                        "no_hp": no_hp,
                        "alamat_pengantaran": alamat_pengantaran if alamat_pengantaran else "-",
                        "menu_pesanan": menu_pesanan,
                        "jumlah": int(jumlah),
                        "total_pesanan": int(total_pesanan),
                        "ongkir": int(ongkir),
                        "metode_pesanan": metode_pesanan,
                        "nama_petugas": nama_petugas if nama_petugas else "-",
                        "status_pengantaran": status_pengantaran,
                        "status_pembayaran_ongkir": status_pembayaran_ongkir,
                        "estimasi_sampai": estimasi_sampai if estimasi_sampai else "-",
                        "catatan": catatan if catatan else "-",
                        "updated_at": now_string()
                    }
                    add_order(new_order)
                    st.success(f"Pesanan berhasil disimpan dengan ID: {new_order['order_id']}")
                    st.info(f"Total pesanan: {rupiah(total_pesanan)} | Ongkir: {rupiah(ongkir)} | Total bayar: {rupiah(total_bayar)}")

    # =========================
    # ADMIN PAGE: TRACKING PESANAN
    # =========================
    elif admin_page == "Tracking Pesanan":
        st.markdown("## 🔎 Tracking Pesanan")
        st.caption("Cari pesanan berdasarkan ID, nama pelanggan, nomor HP, alamat, menu, atau sumber pesanan.")

        keyword = st.text_input("Masukkan kata kunci pencarian", placeholder="Contoh: AN-20260508-1001 / Rina / Online")
        status_filter = st.multiselect("Filter Status Pesanan", STATUS_OPTIONS, default=[])
        payment_filter = st.multiselect("Filter Pembayaran Ongkir", PAYMENT_OPTIONS, default=[])

        filtered = df.copy()

        if keyword:
            keyword_lower = keyword.lower()
            filtered = filtered[
                filtered.apply(
                    lambda row: keyword_lower in " ".join(row.astype(str).str.lower().tolist()),
                    axis=1
                )
            ]

        if status_filter:
            filtered = filtered[filtered["status_pengantaran"].isin(status_filter)]

        if payment_filter:
            filtered = filtered[filtered["status_pembayaran_ongkir"].isin(payment_filter)]

        st.write(f"Jumlah data ditemukan: **{len(filtered)}**")

        if filtered.empty:
            st.warning("Data tidak ditemukan.")
        else:
            for _, row in filtered.sort_values("updated_at", ascending=False).iterrows():
                with st.container(border=True):
                    top1, top2, top3 = st.columns([2, 1, 1])
                    with top1:
                        st.markdown(f"### {row['order_id']} - {row['nama_pelanggan']}")
                        st.caption(f"Sumber: {row['sumber_pesanan']} | Terakhir diperbarui: {row['updated_at']}")
                    with top2:
                        st.markdown(get_status_badge(row["status_pengantaran"]), unsafe_allow_html=True)
                    with top3:
                        st.markdown(get_payment_badge(row["status_pembayaran_ongkir"]), unsafe_allow_html=True)

                    c1, c2, c3 = st.columns(3)
                    with c1:
                        st.write("**Tanggal:**", row["tanggal"])
                        st.write("**No. HP:**", row["no_hp"])
                        st.write("**Menu:**", row["menu_pesanan"])
                        st.write("**Jumlah:**", row["jumlah"])
                    with c2:
                        st.write("**Alamat:**", row["alamat_pengantaran"])
                        st.write("**Kurir/Petugas:**", row["nama_petugas"])
                        st.write("**Metode Pesanan:**", row["metode_pesanan"])
                        st.write("**Estimasi:**", row["estimasi_sampai"])
                    with c3:
                        st.write("**Total Pesanan:**", rupiah(row["total_pesanan"]))
                        st.write("**Ongkir:**", rupiah(row["ongkir"]))
                        st.write("**Total Bayar + Ongkir:**", rupiah(row["total_pesanan"] + row["ongkir"]))
                        st.write("**Catatan:**", row["catatan"])

    # =========================
    # ADMIN PAGE: UPDATE STATUS
    # =========================
    elif admin_page == "Update Status":
        st.markdown("## ✏️ Update Status Pesanan")
        st.caption("Admin/owner dapat mengubah status pesanan, ongkir, estimasi, dan status pembayaran.")

        if df.empty:
            st.info("Belum ada data pesanan.")
        else:
            selected_order = st.selectbox(
                "Pilih Order ID",
                df.sort_values("updated_at", ascending=False)["order_id"].tolist()
            )

            order_data = df[df["order_id"] == selected_order].iloc[0]

            with st.container(border=True):
                st.markdown(f"### Detail Saat Ini: {selected_order}")
                st.write("**Sumber Pesanan:**", order_data["sumber_pesanan"])
                st.write("**Nama Pelanggan:**", order_data["nama_pelanggan"])
                st.write("**Menu:**", order_data["menu_pesanan"])
                st.write("**Jumlah:**", order_data["jumlah"])
                st.write("**Total Pesanan:**", rupiah(order_data["total_pesanan"]))
                st.write("**Ongkir:**", rupiah(order_data["ongkir"]))
                st.write("**Status Pesanan:**", order_data["status_pengantaran"])
                st.write("**Status Pembayaran Ongkir:**", order_data["status_pembayaran_ongkir"])

            with st.form("form_update_status"):
                c1, c2 = st.columns(2)

                with c1:
                    new_status = st.selectbox(
                        "Update Status Pesanan",
                        STATUS_OPTIONS,
                        index=STATUS_OPTIONS.index(order_data["status_pengantaran"])
                        if order_data["status_pengantaran"] in STATUS_OPTIONS else 0
                    )
                    new_payment = st.selectbox(
                        "Update Status Pembayaran Ongkir",
                        PAYMENT_OPTIONS,
                        index=PAYMENT_OPTIONS.index(order_data["status_pembayaran_ongkir"])
                        if order_data["status_pembayaran_ongkir"] in PAYMENT_OPTIONS else 0
                    )
                    new_ongkir = st.number_input(
                        "Update Ongkir / Bill Pengantaran (Rp)",
                        min_value=0,
                        value=int(order_data["ongkir"]),
                        step=1000
                    )
                    new_eta = st.text_input("Update Estimasi", value=str(order_data["estimasi_sampai"]))

                with c2:
                    new_order_method = st.selectbox(
                        "Metode Pesanan",
                        ORDER_METHOD_OPTIONS,
                        index=ORDER_METHOD_OPTIONS.index(order_data["metode_pesanan"])
                        if order_data["metode_pesanan"] in ORDER_METHOD_OPTIONS else 0
                    )
                    new_staff = st.text_input("Nama Kurir / Petugas", value=str(order_data["nama_petugas"]))
                    new_notes = st.text_area("Catatan", value=str(order_data["catatan"]))

                    st.info(f"Total bayar terbaru: {rupiah(int(order_data['total_pesanan']) + int(new_ongkir))}")

                update_btn = st.form_submit_button("Simpan Perubahan", type="primary", use_container_width=True)

                if update_btn:
                    success = update_order(
                        selected_order,
                        {
                            "status_pengantaran": new_status,
                            "status_pembayaran_ongkir": new_payment,
                            "ongkir": int(new_ongkir),
                            "estimasi_sampai": new_eta,
                            "metode_pesanan": new_order_method,
                            "nama_petugas": new_staff,
                            "catatan": new_notes,
                        }
                    )
                    if success:
                        st.success("Status pesanan berhasil diperbarui.")
                        st.rerun()
                    else:
                        st.error("Order ID tidak ditemukan.")

            st.divider()
            st.markdown("### Hapus Data Pesanan")
            st.warning("Gunakan fitur ini hanya jika data salah input.")
            confirm_delete = st.checkbox(f"Saya yakin ingin menghapus {selected_order}")
            if confirm_delete:
                if st.button("Hapus Pesanan", type="secondary"):
                    delete_order(selected_order)
                    st.success("Data pesanan berhasil dihapus.")
                    st.rerun()

    # =========================
    # ADMIN PAGE: DATA & EXPORT
    # =========================
    elif admin_page == "Data & Export":
        st.markdown("## 📦 Data & Export")
        st.caption("Gunakan halaman ini untuk melihat, mengedit langsung, dan mengunduh data pesanan.")

        st.markdown("### Tabel Data Pesanan")
        edited_df = st.data_editor(
            df,
            use_container_width=True,
            hide_index=True,
            num_rows="dynamic",
            column_config={
                "status_pengantaran": st.column_config.SelectboxColumn(
                    "status_pengantaran",
                    options=STATUS_OPTIONS,
                    required=True
                ),
                "status_pembayaran_ongkir": st.column_config.SelectboxColumn(
                    "status_pembayaran_ongkir",
                    options=PAYMENT_OPTIONS,
                    required=True
                ),
                "metode_pesanan": st.column_config.SelectboxColumn(
                    "metode_pesanan",
                    options=ORDER_METHOD_OPTIONS,
                    required=True
                ),
                "total_pesanan": st.column_config.NumberColumn(
                    "total_pesanan",
                    format="Rp %d"
                ),
                "ongkir": st.column_config.NumberColumn(
                    "ongkir",
                    format="Rp %d"
                ),
            }
        )

        c1, c2, c3 = st.columns(3)
        with c1:
            if st.button("Simpan Perubahan Tabel", type="primary", use_container_width=True):
                edited_df["updated_at"] = now_string()
                save_data(edited_df)
                st.success("Perubahan tabel berhasil disimpan.")
                st.rerun()

        with c2:
            st.download_button(
                "Download CSV",
                data=make_csv_download(df),
                file_name="ayam_nyakot_tracking.csv",
                mime="text/csv",
                use_container_width=True
            )

        with c3:
            st.download_button(
                "Download Excel",
                data=make_excel_download(df),
                file_name="ayam_nyakot_tracking.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True
            )
